...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        p = str(Path(path).resolve())  # os-agnostic absolute path  返回p的绝对路径
        if '*' in p:
        ...
        self.nf = ni + nv  # number of files    # 计算图片和视频的总数
        # 设置判断是否为视频的bool变量，方便后面单独对视频进行处理
        self.video_flag = [False] * ni + [True] * nv
        # 初始化模块信息，代码中对mode=images和mode=video有不同处理
        self.mode = 'image'
        self.auto = auto
    ...
    def __next__(self):
        if self.count == self.nf:           # self.count==self.nf 表示数据读取完了
            raise StopIteration
        path = self.files[self.count]       # 获取图片路径

        if self.video_flag[self.count]:     # 如果该文件是视频文件
            # Read video
            self.mode = 'video'
            ret_val, img0 = self.cap.read()     # 获取当前帧画面，ret_val为一个bool变量，直到视频读取完毕之前都为True
            while not ret_val:      # 如果当前视频读取结束，则读取下一个视频文件
                self.count += 1
        ...
        # Padded resize
        img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0]     # 对图片进行resize+pad

        # Convert
        # OpenCV读入的图像BGR -> RGB操作；BGR转为RGB格式，并且把channel轴换到前面
        # torch.Tensor 高维矩阵的表示: (batch_size) x C x H x W
        # numpy.ndarray 高维矩阵的表示: H x W x C
        # 把channel轴换到前面使用transpose() 方法
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB  等价操作 -> img_n[:, :, ::-1].transpose(2, 0, 1)
        img = np.ascontiguousarray(img)     # 将数组内存转为连续，提高运行速度

        return path, img, img0, self.cap, s # 返回：路径，resize+pad图片，原始图片，视频对象

    def new_video(self, path):
        self.frame = 0                      # frame用来记录帧数
        self.cap = cv2.VideoCapture(path)   # 初始化视频对象
        self.frames = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))   # 视频文件的总帧数
...
# 自定义数据集
# 定义LoadImagesAndLabels类，继承Dataset，重写魔法方法：__getitem__(),__len__()
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride  # 模型下采样的步长 ---> 下采样的倍数
        self.path = path    # 图片路径
        self.albumentations = Albumentations() if augment else None     # 如果安装了Albumentations库就会调用它的数据增强

        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:
                # 获取数据集路径path，包含图片路径的txt或者包含图片的文件夹路径
                # 使用pathlib.Path生成与操作系统无关的路径，因为不同操作系统路径的'/'会有所不同
                p = Path(p)  # os-agnostic
                if p.is_dir():  # dir，如果路径p为包含图片的文件夹路径
                    # 同时获取所有的匹配路径下的所有文件
                    ...
                    with open(p) as t:
                        ...
                        parent = str(p.parent) + os.sep  # 获取数据集路径的上级父类目录，os.sep为路径里的分隔符（不同系统的路径分隔符不同，os.sep根据系统自适应）
                        f += [x.replace('./', parent) if x.startswith('./') else x for x in t]  # local to global path
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # local to global path (pathlib)
                else:
                    raise Exception(f'{prefix}{p} does not exist')
            # 分隔符替换为os.sep，x.split('.')将文件名用 ‘.’ 分割，在“.”号处分隔开并返回为一个列表，然后利用sorted()对文件名进行排序
        ...
        # Check cache
        # 利用img2label_paths()函数，由给定的图片路径得到图片标签路径
        self.label_files = img2label_paths(self.img_files)  # labels
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_suffix('.cache')
        try:
            # 将标签都加载到内存中，这样训练时就不用每次去读txt文件
    ...
    def __getitem__(self, index):
        ...
        """
        self.indices在train.py中设置，要配合着train.py的代码来使用
        """
        ...
        if mosaic:
            # Load mosaic
            img, labels = self.load_mosaic(index)  # 使用mosaic数据增强方式加载图片和标签
            shapes = None
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                # 根据pad调整框的标签坐标，并从归一化的xywh -> 未归一化的xywh     pad[0]->pad width, pad[1]->pad height
        ...
        if self.augment:
            ...
            # Flip up-down      图片随机上下反转
            if random.random() < hyp['flipud']:
                img = np.flipud(img)
                if nl:
                    labels[:, 2] = 1 - labels[:, 2]

            # Flip left-right   图片随机左右反转
        ...
        # Convert
        # img.transpose()[::-1]的作用就是实现BGR到RGB通道的转换；对于列表img进行img[:, :,  ::-1]的作用是使列表数组左右翻转
        # channel轴换到前面
    ...
    # 加载图片并根据设定的大小与图片原大小的比例ratio进行resize
    ...
    # 生成一个mosaic增强图片
    def load_mosaic(self, index):  # index --> 图片的下标
        ...
        # 进行mosaic的时候，将四张图片整合到一起以后shape为[2*img_size, 2*img_size]
    ...
    # 例如，通过重写此函数实现标签与图片对应的划分，一个batch中哪些标签属于哪一种图片
