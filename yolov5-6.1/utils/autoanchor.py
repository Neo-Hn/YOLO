...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    """
        dataset: 数据集
        model: 模型
        thr: 阈值，在超参数文件中有，由hyp['anchor_t']得到
        imgsz: image size 图片大小
    """
    ...
    bpr, aat = metric(anchors.cpu().view(-1, 2))    # 列数等于2，行数自动推理
    s = f'\n{PREFIX}{aat:.2f} anchors/target, {bpr:.3f} Best Possible Recall (BPR). '
    if bpr > 0.98:  # threshold to recompute
        LOGGER.info(emojis(f'{s}Current anchors are a good fit to dataset ✅'))
    else:           # 如果bpr小于98%，则根据k-means算法聚类新的锚框
...
def kmean_anchors(dataset='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def metric(k, wh):  # compute metrics
        r = wh[:, None] / k[None]           # wh是图片大小，k是锚框大小
    ...
    # Filter
    i = (wh0 < 3.0).any(1).sum()    # wh0小于三个像素，那就是很小的目标
    if i:
    ...
    k = kmeans(wh / s, n, iter=30)[0] * s  # points；使用scipy包的kmeans函数
