...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16):
    ...
    # Sort by objectness
    # np.argsort() -> 按照矩阵的值从小到大排序，输出的是由排序后的值所对应的序号组成的ndarray
    i = np.argsort(-conf)
    # conf是置信度得分，pred_cls是预测类别，两者的shape是一样的，均是由anchor得来
    tp, conf, pred_cls = tp[i], conf[i], pred_cls[i]

    # Find unique classes
    # unique_classes 即不同的类别数，一般等于数据集的类别数
    unique_classes, nt = np.unique(target_cls, return_counts=True)
    nc = unique_classes.shape[0]  # number of classes, number of detections

    # Create Precision-Recall curve and compute AP for each class
    px, py = np.linspace(0, 1, 1000), []  # for plotting
    # ap的shape是[类别数, '从0.5-0.95 -> 10个']
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_l == 0:
            ...
            # Recall
            # n_l 就是 TP + FN    recall = TP / (TP + FN)
            recall = tpc / (n_l + eps)  # recall curve
            r[ci] = np.interp(-px, -conf[i], recall[:, 0], left=0)  # negative x, xp because xp decreases

            # Precision
            # tpc + fpc = TP + FP   precision = TP / (TP + FP)
...
def compute_ap(recall, precision):
    ...
    # np.maximum.accumulate(numpy.array([11, 12, 13, 20, 19, 18, 17, 18, 23, 21]))
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU:
        # cw、ch -> 将box1和box2围起来的最小宽高
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        # 这一部分的计算需要根据CIoU或DIoU的公式来
...
def box_iou(box1, box2):
    ...
    """
    Return intersection-over-union (Jacquard index) of boxes.
    Both sets of boxes are expected to be in (x1, y1, x2, y2) format.
    Arguments:
        box1 (Tensor[N, 4])
        box2 (Tensor[M, 4])
