...
# 检查图片尺寸是否是s（默认是32）的整数倍；不是则调整图像尺寸
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    """Runs Non-Maximum Suppression (NMS) on inference results
        Args:
            prediction: 预测信息 -> shape = [1, 预测框数目, 类别数+5] -> (类别数+5 = xywh + 置信度(是否有物体) + 类别置信度(size=nc)) 注意: 都是按顺序排列的
            conf_thres: 置信度阈值
            iou_thres: IoU阈值
            max_det: 单张图片上最多检测的物体数
    Returns:
         list of detections, on (n,6) tensor per image [xyxy, conf, cls]
    """

    # nc  ->  类别数
    # xc  ->  筛选出预测框中置信度大于置信度阈值的预测框，使其成为候选框
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            # 在pytorch中max()方法较为特别，我得写个博客
            # conf -> 挑选出每一行最大的置信度，也就是选出置信度最大的类别物体
            # j    -> 置信度最大的类别物体所对应的下标index
            conf, j = x[:, 5:].max(1, keepdim=True)
            # 最终x包含 -> [左上角坐标, 右下角坐标, conf 1个, conf的索引] -> 列维度=6
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    ...
    if path.exists() and not exist_ok:
        # path.with_suffix() : 返回更改 suffix 的新路径。如果原始路径没有后缀，则会追加新的suffix。如果 suffix 是空字符串，则删除原始后缀
